...
const { LeagueShopDB } = require('../db/leagueshop.js');
...
const LeagueShopCron = async (client) => {
    ...
    // Get shop ID from DB
    const shopData = await LeagueShopDB.findOne({
        where: {
            id: 1
        }
    });
