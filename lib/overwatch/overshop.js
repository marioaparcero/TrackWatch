...
const OverShop = async () => { 
  ...
  //   itemsData += `* ${i.title.replace("Overwatch®: ", "")} - **${i.price.raw} ${i.price.currency !== "GBP" ? '\<:moneda:1044000893726625823>' : ''}${i.price.currency == 'GBP' ? 'EUR' : ''}**\n`; // ${Math.floor(i.price.raw)}
  ...
  for (const i of result.mtxCollections[0].items) {
    ...
    if (i.price.currency === "GBP") {
      ...
      // itemsData += `* ${i.title.replace("Overwatch®: ", "")} - **${precioEUR} EUR**\n`;
      itemsData += `* ${i.title.replace("Overwatch®: ", "")} - **${precioEUR} ${i.price.currency !== "GBP" ? '\<:moneda:1044000893726625823>' : 'EUR'}**\n`;
      // itemsData += `* ${i.title.replace("Overwatch®: ", "")} - **${precioEUR} ${i.price.currency !== "GBP" ? '\<:moneda:1044000893726625823>' : ''}${i.price.currency == 'GBP' ? 'EUR' : ''}**\n`;
    } else {
      // Si el precio ya está en EUR, mostrarlo directamente
      // itemsData += `* ${i.title.replace("Overwatch®: ", "")} - **${i.price.raw} EUR**\n`;
      itemsData += `* ${i.title.replace("Overwatch®: ", "")} - **${i.price.raw} ${i.price.currency !== "GBP" ? '\<:moneda:1044000893726625823>' : 'EUR'}**\n`;
      // itemsData += `* ${i.title.replace("Overwatch®: ", "")} - **${i.price.raw} ${i.price.currency !== "GBP" ? '\<:moneda:1044000893726625823>' : ''}${i.price.currency == 'GBP' ? 'EUR' : ''}**\n`;
    }
  }

  for (const i of card) {
    if (i.slug.match(/(battle-pass)|(starter-pack)|(tier-skip)|(hero)|(ultimate)/g)) { // US$, EUR, GBP
      // Regla especial: Si es el pase de batalla y el precio es 0, poner 1000
      let displayPrice = i.price.raw;
      if (displayPrice === 0) { // i.categoryId === 'battle-pass'
        displayPrice = 1000;
      }
      seasonCard += `* ${i.title.replace("Overwatch®: ", "")} - **${displayPrice} ${i.price.currency !== "USD" ? '\<:moneda:1044000893726625823>' : ''}${i.price.currency == 'USD' ? 'EUR' : ''}**\n`;
...
const OverShopCron = async (client) => {
  try {
    ...
    const embed = new EmbedBuilder()
      .setAuthor({
        name: 'Overwatch',
        iconURL: 'https://images-ext-1.discordapp.net/external/tTKzALJXJSHXWduLkHt9hT_d_obdeFHQ_cyx5-EpIQ8/https/cdn.patchbot.io/games/106/overwatch_2_sm.webp?format=webp&width=312&height=312',
