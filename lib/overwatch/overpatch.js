...
try {
  ...
  if (!heroePatch) {
    ...
    if (titlePatch[1] === 'TANK') { titlePatch[1] = 'TANQUE' };
        ...
        // ----------------------------------------------------------------------------------------------------------
        // // OPCIÓN 3 - (Orden todo por rol y agrupando habilidades de los heroes, sin tener en cuenta el origen (Tanque > Dva, Roadhog, Daño > Hanzo, Genji))
        // // Declaramos la constante para almacenar los datos de los héroes
        // const heroes = {};
        ...
        // ----------------------------------------------------------------------------------------------------------
        // (OPCIÓN 5) - (Ordenado por rol respetando el mismo orden del origen) (Tanque > Dva, Roadhog, Daño > Hanzo, Genji))  (Sin las descripciones generales
        // Declaramos la constante para almacenar los datos de los héroes
        const heroes = {
            TANQUE: [],
            DAÑO: [],
        ...
        sectionTitles.each((index, element) => {
            ...
            heroesInSection.each((i, el) => {
                ...
                // Agregar el héroe al arreglo correspondiente según su rol
                if (heroes[sectionTitle]) {
                    heroes[sectionTitle].push({ name: heroName, data: heroData });
                }
            });
        });
        ...
        // Imprimir los datos y los títulos de las secciones
        Object.keys(heroes).forEach(sectionTitle => {
            const sectionHeroes = heroes[sectionTitle];
            if (sectionHeroes.length > 0) {
                descriptionEmbed += `### ${sectionTitle}\n`; // Agregar el título de la sección

                // Iterar sobre los héroes de esta sección
                sectionHeroes.forEach(hero => {
        ...
        // (OPCIÓN 6) - Iterar sobre los datos de los héroes y construir la descripción del embed
    ...
    // Función para dividir el contenido en partes más pequeñas
    ...
    const chunks = chunkString(descriptionEmbed, 4000); // Cambia 1000 al tamaño máximo de caracteres por cada parte de descripción que desees
    console.log(chunks)
    // Embed
