...
const OverShop = async () => { 
  ...
  for (const i of result.mtxCollections[0].items) {
    itemsData += `* ${i.title.replace("Overwatch® 2: ", "")} - **${Math.floor(i.price.raw)} ${i.price.currency !== "USD" ? '\<:moneda:1044000893726625823>' : ''}${i.price.currency == 'USD' ? 'EUR' : ''}**\n`;
