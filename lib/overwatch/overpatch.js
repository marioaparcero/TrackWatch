...
const { formatHeroes, getEmoji } = require("../../utils/emojis");
...
try {
  ...
  if (!heroePatch) {
    ...
    if (titlePatch[1] === 'TANK') { titlePatch[1] = 'TANQUE' };
        ...
        Object.entries(heroesData).forEach(([sectionTitle, heroesInSection]) => { // Si descomento el test de abajo tengo que quitar heroesInSection de esta linea
            ...
            if (heroesInSection !== sectionTitle) {
                let currentSectionEmoji = sectionTitle;
                // Asignar el emoji correspondiente según el título de la sección (Versión 1)
                ...
                // // Mapear nombres de secciones a emojis usando Map, una forma más elegante de hacerlo. (Versión 2)
                ...
                // ]);
                // currentSectionEmoji = sectionMap.get(sectionTitle) || `✨ ${sectionTitle}`;
                
                // // Mapear nombres de secciones a emojis usando objeto, más simple. (Versión 3)
                // const sectionMap = {
                //     'TANQUE': 'TANQUE <:tanque:979726195811811348>',
                //     'Tanques': 'TANQUE <:tanque:979726195811811348>',
                //     'Tanque': 'TANQUE <:tanque:979726195811811348>',
                ...
                //     'Support': 'APOYO <:apoyo:979742776495312956>'
                // };
                // currentSectionEmoji = sectionMap[sectionTitle] || `✨ ${sectionTitle}`;
                
                // // Mapear nombres de secciones a emojis usando un objeto (Versión 4)
                // const EMOJI_TANQUE = 'TANQUE <:tanque:979726195811811348>';
                // const EMOJI_DPS = 'DAÑO <:dps:979742754575896669>';
                // const EMOJI_APOYO = 'APOYO <:apoyo:979742776495312956>';

                // const sectionMap = {
                //     'TANQUE': EMOJI_TANQUE,
                //     'Tanques': EMOJI_TANQUE,
                //     'Tanque': EMOJI_TANQUE,
                //     'Tank': EMOJI_TANQUE,
                //     'DAÑO': EMOJI_DPS,
                //     'Daños': EMOJI_DPS,
    ...
    //     url: patchurl,
    //     // title: `Información del parche de Overwatch - ${patchtime.getDate()}/${patchtime.getMonth() + 1}/${patchtime.getFullYear()}`,
    //     title: `${datePatch}`,
    ...
    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
        .setLabel('Ir a las notas del parche de Overwatch')
        .setStyle('Link')
        .setURL(patchurl),
    );
...
const OverPatch = async () => {
    try {
        const patchhtmlNext = await GetPatchData(language);
        const match = patchhtmlNext.match("No se han encontrado notas del parche");
        let patchhtml = '';
        ...
        if (!match) {
            ...
            const patchhtmlNow = await GetPatchDataNow(language);
            patchhtml = patchhtmlNow;
        }

        if (!patchhtml) return null;

        // Extraer información básica del parche
        const matches = patchhtml.match(/(?<=id=\")([^"]*)(?=\")/g);
        if (!matches || matches.length === 0) {
            console.error("❌ No se encontró ningún atributo id con el formato patch-YYYY-MM-DD.");
            return null;
        ...
        const conv = matches[0];
        const date = conv.replace('patch-', '');
        const patchtime = new Date(date);
        const patchurl = `https://overwatch.blizzard.com/news/patch-notes/live/${date.split('-')[0]}/${date.split('-')[1]}/#patch-${date}`;

        // Extraer la fecha formateada
        const datePatchMatch = patchhtml.match(/(?<=PatchNotes-date">)([^<]*)/g);
        const datePatch = datePatchMatch ? datePatchMatch[0] : "Fecha no disponible";

        // Extraer títulos
        const titlePatchMatch = patchhtml.match(/(?<=PatchNotes-sectionTitle">)([^<]*)/g);
        const titlePatch = titlePatchMatch || ["Título no disponible"];

        // Extraer descripciones
        const descriptionPatchMatch = patchhtml.match(/(?<=<div class="PatchNotes-sectionDescription"><p>)(.*?)(?=<\/p>)/sg);
        const descriptionPatch = descriptionPatchMatch || ["Descripción no disponible"];

        // Usar cheerio para procesar el HTML más eficientemente
        const $ = cheerio.load(patchhtml);
        ...
        // Extraer datos de héroes si existen
        const heroePatchMatch = patchhtml.match(/(?<=PatchNotesHeroUpdate-name">)([^<]*)/g);
        
        if (heroePatchMatch && heroePatchMatch.length > 0) {
            // Si hay actualizaciones de héroes, procesarlas
            const sectionTitles = $('.PatchNotes-sectionTitle');
                ...
                heroesInSection.each((i, el) => {
                    ...
                    if (abilitiesList.length > 0) {
                        abilitiesList.find('.PatchNotesAbilityUpdate').each((index, abilityElement) => {
                            ...
                            if (!heroesData[heroName].abilities[abilityName]) {
                                heroesData[heroName].abilities[abilityName] = {
                                    name: abilityName,
                                    details: abilityDetails
                                };
                            } else {
                                heroesData[heroName].abilities[abilityName].details += `\n${abilityDetails}`;
                            }
                        });
                    }
                    ...
                    if (generalUpdates.length > 0) {
                        generalUpdates.find('li').each((index, updateElement) => {
                            heroesData[heroName].generalUpdates.push($(updateElement).text().trim());
                        });
                    }
                });
            });
            
            // Agrupar héroes por sección
            const heroesGrouped = {};
            Object.entries(heroesData).forEach(([heroName, heroData]) => {
                const sectionTitle = heroData.section;
                if (!heroesGrouped[sectionTitle]) {
                    heroesGrouped[sectionTitle] = [];
                }
                heroesGrouped[sectionTitle].push({ name: heroName, ...heroData });
            });
            
            // Construir la descripción con actualizaciones de héroes
            if (titlePatch[0]) {
                descriptionEmbed += `## ${titlePatch[0]}\n`;
            }
            
            Object.entries(heroesGrouped).forEach(([sectionTitle, heroesInSection]) => {
                let currentSectionEmoji = sectionTitle;
                
                // Mapear nombres de secciones a emojis usando un objeto
                const EMOJI_TANQUE = 'TANQUE <:tanque:979726195811811348>';
                const EMOJI_DPS = 'DAÑO <:dps:979742754575896669>';
                const EMOJI_APOYO = 'APOYO <:apoyo:979742776495312956>';
                ...
                currentSectionEmoji = sectionMap[sectionTitle] || `✨ ${formatHeroes(sectionTitle)}`;
                descriptionEmbed += `## ${currentSectionEmoji}\n`;
                ...
                heroesInSection.forEach(heroData => {
                    ...
                    // Agregar habilidades
                    Object.values(heroData.abilities).forEach(ability => {
                        const abilityName = ability.name || 'Habilidad';
                        descriptionEmbed += `- __${abilityName}__\n`;
                    ...
                    // Agregar actualizaciones generales
                    heroData.generalUpdates.forEach(update => {
                        if (update.trim()) {
                            descriptionEmbed += `- ${update}\n`;
                        }
            ...
            // Si no hay actualizaciones de héroes, mostrar información general
            if (titlePatch[0] && descriptionPatch[0]) {
                descriptionEmbed += `**${titlePatch[0]}**\n${descriptionPatch[0]}\n`;
            }
            
            if (titlePatch[1] && descriptionPatch[1]) {
                descriptionEmbed += `**${titlePatch[1]}**\n${descriptionPatch[1]}\n`;
            }
        }
        ...
        return {
            ...
            date: datePatch,
            patchUrl: patchurl,
            description: descriptionEmbed.trim(),
            hasHeroUpdates: Object.keys(heroesData).length > 0
        };
        
    } catch (error) {
        console.error('Error en OverPatch:', error);
        return null;
    }
};

module.exports = { OverPatchCron, OverPatch }; // GetPatchData, GetPatchDataNow, (TODO: Para exportarlo en el futuro para usar en un comando)